...
export function browserUuid() {
  ...
  if (typeof api?.getRandomValues === "function") {
    return bytesToUuid(api.getRandomValues(new Uint8Array(16)));
  }

  fallbackCounter += 1;
  const seed = [
    Date.now().toString(16),
    fallbackCounter.toString(16),
...
function sha256Fallback(input: Uint8Array) {
  ...
  const message = new Uint8Array(input.length + 1 + paddingLength + 8);
  message.set(input);
...
export async function sha256Hex(input: Uint8Array) {
  const subtle = globalThis.crypto?.subtle;
  if (subtle) {
    try {
      const digest = await subtle.digest("SHA-256", input.slice().buffer);
      return Array.from(new Uint8Array(digest), (value) =>
        value.toString(16).padStart(2, "0")
      ).join("");
    } catch {
      // Plain HTTP and some embedded browsers expose crypto without usable SubtleCrypto.
    }
  }
