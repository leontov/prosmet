...
function safeName(value: string) {
  return value.replace(/[^a-zA-Zа-яА-Я0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 90) || "prosmet-document";
}
...
export function DocumentEditor({
  ...
  const fallbackId = useRef(`document_${crypto.randomUUID()}`).current;
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      const next = stored ?? { ...incoming, threadId: workspace.currentThreadId, content: html(incoming.content) };
      setDocument(next);
      initialized.current = next.id;
      if (!stored) await repository.saveDocument(next);
      setSaved(true);
    })().catch((reason) => setError(reason instanceof Error ? reason.message : "Не удалось открыть документ"));
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    if (editorRef.current.innerHTML !== document.content) editorRef.current.innerHTML = document.content;
  ...
  return (
    <section
      ...
      className={cn(
        ...
        fullscreen ? "fixed inset-0 z-[110] h-dvh flex-col rounded-none" : "mt-3 w-full max-w-(--thread-max-width) flex-col rounded-2xl"
      )}
    >
      <header className="flex flex-col gap-3 border-b border-neutral-200 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-xs text-neutral-500">
            ...
            <span className="rounded-full border border-neutral-200 px-2 py-0.5">Версия {document.revision}</span>
            <span className={cn("rounded-full px-2 py-0.5", document.status === "approved" ? "bg-emerald-50 text-emerald-700" : "bg-neutral-100")}>{document.status === "approved" ? "Утверждён" : "Черновик"}</span>
            {saved && <span className="inline-flex items-center gap-1 text-emerald-700"><CheckIcon className="size-3.5" />SQLite</span>}
          </div>
          <input
            value={document.title}
            onChange={(event) => {
              setSaved(false);
              setDocument((current) => (current ? { ...current, title: event.target.value } : current));
            }}
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={downloadDoc} className="inline-flex h-9 items-center gap-2 rounded-lg border border-neutral-200 bg-white px-3 text-sm font-medium hover:bg-neutral-50"><DownloadIcon className="size-4" />DOC</button>
          <button type="button" onClick={() => window.print()} className="inline-flex h-9 items-center gap-2 rounded-lg border border-neutral-200 bg-white px-3 text-sm font-medium hover:bg-neutral-50"><PrinterIcon className="size-4" />PDF / печать</button>
          <button type="button" onClick={() => setFullscreen((value) => !value)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-neutral-200 bg-white px-3 text-sm font-medium hover:bg-neutral-50">{fullscreen ? <XIcon className="size-4" /> : null}{fullscreen ? "Свернуть" : "Развернуть"}</button>
          <button type="button" disabled={busy} onClick={() => void save("approved")} className="inline-flex h-9 items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 text-sm font-medium text-emerald-700 hover:bg-emerald-100 disabled:opacity-50"><BadgeCheckIcon className="size-4" />Утвердить</button>
          <button type="button" disabled={busy} onClick={() => void save()} className="inline-flex h-9 items-center gap-2 rounded-lg bg-neutral-900 px-3 text-sm font-medium text-white hover:bg-black disabled:opacity-50">{busy ? <LoaderCircleIcon className="size-4 animate-spin" /> : <SaveIcon className="size-4" />}{saved ? "Сохранено" : "Сохранить"}</button>
        </div>
      </header>
      ...
      <div className="prosmet-scrollbar min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
        <div className="mx-auto grid max-w-[1080px] gap-5 xl:grid-cols-[minmax(0,794px)_250px]">
          <article className="print-page min-h-[1123px] bg-white px-[8%] py-[8%] shadow-[0_1px_6px_rgba(0,0,0,0.16)]">
            <h1 className="mb-10 text-center text-xl font-semibold leading-tight">{document.title}</h1>
            <div
              ref={editorRef}
              contentEditable
              ...
              onInput={(event) => {
                setSaved(false);
                setDocument((current) => (current ? { ...current, content: event.currentTarget.innerHTML } : current));
              }}
            ...
            <section className="rounded-2xl border border-neutral-200 bg-white p-4">
              ...
              {document.missingFields.length ? (
                <ul className="mt-3 space-y-2 text-sm text-amber-800">
                  {document.missingFields.map((field) => <li key={field} className="rounded-lg bg-amber-50 px-3 py-2">{field}</li>)}
                </ul>
              ) : (
                <p className="mt-2 text-sm leading-6 text-emerald-700">Обязательные пропуски не заявлены. Перед печатью проверьте реквизиты, даты и суммы.</p>
              )}
            </section>
          </aside>
        </div>
