...
export function ChatScreen({ hasEstimate, onOpenEstimate }: Props) {
  return (
    <View style={styles.screen}>
      <View style={styles.topbar}><View style={styles.mark}><Text style={styles.markText}>✦</Text></View><Text style={styles.brand}>Просметчик</Text><View style={styles.topbarSpacer} /></View>
              ...
              <View style={styles.suggestions}>
                {suggestions.map(([title, prompt], index) => (
                  <ThreadPrimitive.Suggestion key={title} prompt={prompt} send style={styles.suggestion}>
                    <View style={styles.suggestionIcon}><Text style={styles.suggestionIconText}>{index === 0 ? "⌂" : index === 1 ? "▤" : "□"}</Text></View>
                    <View style={styles.suggestionCopy}><Text style={styles.suggestionTitle}>{title}</Text><Text style={styles.suggestionText}>{prompt}</Text></View>
          ...
          <View style={styles.messages}>
            <ThreadPrimitive.Messages>
              {({ message }) => message.role === "user" ? (
                <MessagePrimitive.Root style={styles.userMessage}><View style={styles.userBubble}><MessagePrimitive.Parts /></View></MessagePrimitive.Root>
              ) : (
                <MessagePrimitive.Root style={styles.assistantMessage}><View style={styles.assistantAvatar}><Text>✦</Text></View><View style={styles.assistantCopy}><MessagePrimitive.Parts /></View></MessagePrimitive.Root>
              )}
            </ThreadPrimitive.Messages>
            {hasEstimate ? (
              <Text onPress={onOpenEstimate} accessibilityRole="button" style={styles.artifact}>▤  Смета готова · открыть и проверить расчёт</Text>
            ) : null}
          </View>

          <ThreadPrimitive.ViewportFooter style={styles.footer}>
            <ComposerPrimitive.Root style={styles.composer}>
              <Text style={styles.attach}>＋</Text>
              <ComposerPrimitive.Input style={styles.input} placeholder="Сообщение Просметчику" placeholderTextColor={theme.faint} multiline />
...
const styles = StyleSheet.create({
  ...
  thread: { flex: 1 },
  viewport: { flex: 1 },
  welcome: { paddingHorizontal: 16, paddingTop: 30, paddingBottom: 150 },
  ...
  messages: { paddingHorizontal: 14, paddingTop: 20, paddingBottom: 130, gap: 22 },
  ...
  artifact: { minHeight: 86, overflow: "hidden", borderWidth: 1, borderColor: theme.border, borderRadius: 18, backgroundColor: theme.canvas, padding: 17, color: theme.text, fontSize: 16, lineHeight: 24, fontWeight: "700" },
  footer: { marginTop: "auto", paddingHorizontal: 12, paddingBottom: 14, paddingTop: 12, backgroundColor: theme.canvas },
