...
function providerLabel(type: AgentProviderKind) {
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <Section title="Соединение с сервером">
        <Field label="API URL"><TextInput style={styles.input} value={apiUrl} onChangeText={setApiUrl} autoCapitalize="none" autoCorrect={false} keyboardType="url" placeholder="https://kolibriai.online" placeholderTextColor={theme.faint} /></Field>
        <Field label="Токен супер-администратора"><TextInput style={styles.input} value={token} onChangeText={setToken} autoCapitalize="none" autoCorrect={false} secureTextEntry placeholder="Хранится в SecureStore" placeholderTextColor={theme.faint} /></Field>
        <PrimaryButton busy={busy === "credentials"} label="Сохранить и проверить" onPress={saveConnection} />
      </Section>
      ...
      <Section title="Новое подключение">
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.providerTabs}>
          {(Object.keys(providerLabels) as AgentProviderKind[]).map((provider) => (
            <Pressable key={provider} style={[styles.providerTab, type === provider && styles.providerTabActive]} onPress={() => setType(provider)}>
              <Text style={[styles.providerTabText, type === provider && styles.providerTabTextActive]}>{providerLabel(provider)}</Text>
            </Pressable>
          ))}
        </ScrollView>
        <Field label="Название"><TextInput style={styles.input} value={name} onChangeText={setName} placeholder="Например, Codex Primary" placeholderTextColor={theme.faint} /></Field>
        {type === "codex-app-server" ? (
          ...
          <Field label={type === "http-agent" ? "Endpoint" : "Base URL"}><TextInput style={styles.input} value={agentUrl} onChangeText={setAgentUrl} autoCapitalize="none" autoCorrect={false} keyboardType="url" placeholder={type === "ollama" ? "http://127.0.0.1:11434" : "https://api.example.com/v1"} placeholderTextColor={theme.faint} /></Field>
        )}
        ...
        <PrimaryButton busy={busy === "create"} label="Подключить агента" disabled={!name.trim()} onPress={create} />
      </Section>
    </ScrollView>
  );
}
...
function AgentCard({ agent, result, busy, onTest, onActivate, onDelete }: {
  ...
  return (
    <View style={[styles.agentCard, agent.active && styles.agentCardActive]}>
      <View style={styles.agentHeader}>
        <View style={[styles.statusDot, agent.active && styles.statusDotActive]} />
        <View style={styles.agentCopy}>
          <Text style={styles.agentName}>{agent.name}</Text>
          <Text style={styles.agentDetail}>{providerLabel(agent.type)}{agent.model ? ` · ${agent.model}` : ""}</Text>
        </View>
        {agent.active ? <Text style={styles.activeLabel}>Активен</Text> : null}
      </View>
      <Text style={styles.endpoint} numberOfLines={3}>{agent.type === "codex-app-server" ? `${agent.command ?? "codex"} ${(agent.args ?? []).join(" ")}` : agent.baseUrl}</Text>
      {result ? <Text style={styles.testResult}>✓ {result.latencyMs} мс · {result.message}</Text> : null}
      <View style={styles.agentActions}>
        <Pressable style={styles.actionButton} onPress={() => void onTest(agent)} disabled={busy === `test:${agent.id}`}>
          {busy === `test:${agent.id}` ? <ActivityIndicator color={theme.text} size="small" /> : <Text style={styles.actionText}>Проверить</Text>}
        </Pressable>
        {!agent.active ? <Pressable style={styles.actionButton} onPress={() => void onActivate(agent)}><Text style={styles.actionText}>Активировать</Text></Pressable> : null}
        <Pressable style={styles.deleteButton} onPress={() => onDelete(agent)}><Text style={styles.deleteText}>Удалить</Text></Pressable>
      </View>
    </View>
  );
}

function PrimaryButton({ busy, label, disabled = false, onPress }: { busy: boolean; label: string; disabled?: boolean; onPress: () => Promise<void> }) {
  return <Pressable style={styles.primaryButton} onPress={() => void onPress()} disabled={busy || disabled}>{busy ? <ActivityIndicator color="white" /> : <Text style={styles.primaryButtonText}>{label}</Text>}</Pressable>;
}
