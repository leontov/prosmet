...
const blankAgent: AgentConfigInput = {
  name: "",
...
const providerLabels: Record<AgentProviderKind, { title: string; detail: string }> = {
  ...
  "codex-app-server": {
    ...
    detail: "Официальный двунаправленный JSONL/stdio runtime Codex"
  },
  "http-agent": {
    title: "HTTP agent",
...
export function SettingsView({ mobile }: { mobile: boolean }) {
  ...
  const [form, setForm] = useState<AgentConfigInput>(blankAgent);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setRegistry({ ...agents, adminAuthenticated: session.authenticated, bootstrapRequired: session.bootstrapRequired });
    } catch (loadError) {
  ...
  const submitAgent = async (event: React.FormEvent) => {
    ...
    try {
      const payload: AgentConfigInput = {
        ...
        ...(form.secret?.trim() ? { secret: form.secret.trim() } : editingId ? {} : { secret: null })
      };
          ...
          <SettingsSection icon={<ServerCogIcon />} title="Активный агент">
            <div className="active-agent-summary">
              ...
              <div>
                ...
                <small>{active ? `${providerLabels[active.type].title}${active.model ? ` · ${active.model}` : ""}` : "Добавьте первое подключение ниже"}</small>
              </div>
          ...
          <SettingsSection icon={<LockKeyholeIcon />} title="Супер-администратор">
            {registry?.adminAuthenticated ? (
              ...
              <form className="admin-login" onSubmit={authenticate}>
                ...
                {registry?.bootstrapRequired ? <p>На сервере используется сгенерированный bootstrap-токен. Он хранится только в защищённом серверном файле.</p> : null}
              </form>
            )}
          </SettingsSection>
                ...
                <div className="agent-form-grid">
                  ...
                  <Field label="Тип интеграции" id="agent-type">
                    <select id="agent-type" name="agent-type" value={form.type} onChange={(event) => setForm((current) => ({ ...current, type: event.target.value as AgentProviderKind }))}>
                      {Object.entries(providerLabels).map(([value, item]) => <option key={value} value={value}>{item.title}</option>)}
                    </select>
                  </Field>
          ...
          {registry?.agents.length ? registry.agents.map((agent) => (
            <article key={agent.id} className={agent.active ? "agent-connection active" : "agent-connection"}>
              <header>
                ...
                <span><strong>{agent.name}</strong><small>{providerLabels[agent.type].title}{agent.model ? ` · ${agent.model}` : ""}</small></span>
                {agent.active ? <CheckCircle2Icon /> : null}
              </header>
              <p>{agent.type === "codex-app-server" ? `${agent.command} ${(agent.args || []).join(" ")}` : agent.baseUrl}</p>
